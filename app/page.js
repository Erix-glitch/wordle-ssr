...
export default async function Page() {
  ...
  return (
    <main className="flex flex-col items-center justify-center min-h-screen bg-gray-900 text-white text-center p-8">
      <h1 className="text-4xl font-bold mb-4">Today's Wordle</h1>
      {solution ? (
        <p className="text-3xl text-green-400">{solution}</p>
      ) : (
        <p className="text-red-400">Could not load today's Wordle.</p>
      )}
    </main>
  );
}
